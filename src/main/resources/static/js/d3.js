highlightActive = !1;
var nodesDataset, fiternode = [], nodeFilterValue = -1,rule_effects,type="Testo",table;
...
function redrawAll(a, c) {
  ...
  network.once("stabilizationIterationsDone", function () {
    ...
    nodeFilterSelector = document.getElementById("slider");
...
$(document).ready(function() {
    ...
    nodeFilterSelector.addEventListener("input", function(a) {
        var filternode = [];
        var nodeFilterValue = parseInt(a.target.value); // Converti il valore del filtro in un numero intero

        console.log(nodeFilterValue);

        nodes.forEach(function(c) {
            if (c.type === "rule" || (nodeFilterValue * c.max / 100) <= c.score) { // Usare || per controllare entrambe le condizioni
                filternode.push(c);
            }
        });
        ...
        var nodesDataset = new vis.DataSet(filternode);
        redrawAll(nodesDataset, edgesDataset);
    });
